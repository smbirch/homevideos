...
public class JwtServiceImpl implements JwtService {
    ...
    public String generateToken(Map<String, Object> claims, String userName) {
        ...
        return Jwts.builder().claims(claims).subject(userName).issuedAt(new Date(now)).expiration(new Date(now + jwtExpiration)).signWith(signInKeyHelper()).compact();
    }
    ...
    public boolean validateTokenAndUser(String token, String username) {
        if (token == null || token.isEmpty()) {
            log.warn("406 - Token is null or empty at validateTokenAndUser");
    ...
    public void setAuthenticationCookie(String token, HttpServletResponse response) {
        ...
        String cookieValue = String.format("%s=%s; Path=/; Max-Age=%d; HttpOnly; SameSite=Lax", "homevideosCookie", token, (int) (jwtExpiration / 1000));
    ...
    private Claims extractAllClaims(String token) {
        return Jwts.parser().verifyWith(signInKeyHelper()).build().parseSignedClaims(token).getPayload();
    }
